...
void ParagraphQuark::updateBounds() {
	...
	size = glm::vec3(getF(WIDTH), lines.size()*(getF(FONT_SIZE) + getF(LINE_SPACING)) + getF(FONT_SIZE) / 2, 0);  // And calculate boundaries
}
...
void ParagraphQuark::freezeToTexture() {
	...
	for (int i = 0; i < lines.size(); i++) {								// For every line,
		...
		if (getUINT(ALIGNMENT) == ALIGN_LEFT)									// And if this paragraph aligns left,
			in += (((s.x/s.y)*getF(FONT_SIZE)/2)-getF(WIDTH)/2);				// Then use the measurement to calculate it's offset
		...
		passes::transform = neutron::processor::rect(in, (i * (getF(FONT_SIZE)+getF(LINE_SPACING))), getF(WIDTH), getF(FONT_SIZE), this->rotation);
