...
namespace neutron {
	namespace world {
		...
		std::map<std::string, std::map<char32_t, Character>> characters = std::map<std::string, std::map<char32_t, Character>>();
		std::map<std::string, int> sizeMap = std::map<std::string, int>();
		FT_Library ft = FT_Library();

		void loadFont(const char* filepath, std::string id, int size, short glyphsNum) {
			...
			std::map<char32_t, Character> characterList = std::map<char32_t, Character>();

			sizeMap.insert(make_pair(id, size));

			for (char32_t c = 0; c < glyphsNum; c++) { // Read char through assigned number of glyphs
		...
		glm::vec2 measureString(const std::string &text, const char* font, float xspacing, float yspacing) {
			return measureString(std::wstring_convert<std::codecvt_utf8<char32_t>, char32_t>().from_bytes(text), font, xspacing, yspacing);
		}
		glm::vec2 measureString(const std::u32string &text, const char* font, float xspacing, float yspacing) {
		...
		void Camera::centerOn(glm::vec3 pos, float smoothness) {
			double ratio = (std::max)(smoothness+frame::delta_time, 0.0);
		...
		void Camera::ProcessMouseScroll(float yoffset) {
			if (moveMode == MODE_ORBIT) {
				...
				OrbitDistance = (std::min)((std::max)(OrbitDistance, OrbitDistanceMin), OrbitDistanceMax);
			}
			else {
