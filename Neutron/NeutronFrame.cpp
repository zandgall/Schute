	...
	namespace frame {
		...
		void swap() {
			...
			if (floor(glfwGetTime()) - lastFrameCount != 0) {
				...
				fps = frames;
				std::cout << "FPS: " << fps << " DELTA TIME: " << floor(delta_time*1000)/1000.0 << "s" << std::endl;
				frames = 0;
			}
		...
		namespace mouse {
			...
			double totalScroll = 0, scroll = 0, x_scroll = 0, y_scroll = 0, x_s = 0, y_s = 0, scroll_glide = 0, x_scroll_glide = 0, y_scroll_glide = 0, scroll_g = 0, x_s_g = 0, y_s_g = 0, GLIDE_DECAY = 0.9;
			bool print = true;
			void lock() {
			...
			void endFrame() {
				...
				y_s_g *= (GLIDE_DECAY - delta_time);
				x_s_g *= (GLIDE_DECAY - delta_time);
		...
		namespace keys {
			...
			std::vector<void(*)(int key, int scancode, int action, int mods)> keyEvents = std::vector<void(*)(int key, int scancode, int action, int mods)>();
			std::vector<void(*)(unsigned int key)> charEvents = std::vector<void(*)(unsigned int key)>();
			bool print = false;
			void beginFrame() {
			...
			void press_call(GLFWwindow* window, int key, int scancode, int action, int mods) {
				...
				for (auto i = keyEvents.begin(); i != keyEvents.end(); i++)
					(*i)(key, scancode, action, mods);
			}
			
			void char_call(GLFWwindow* window, unsigned int key) {
				if (recordTyping)
					if (key == GLFW_KEY_BACKSPACE)
						typedInputString = typedInputString.substr(0, strlen(typedInputString.c_str()) - 1);
					else
						typedInputString += (key == GLFW_KEY_UNKNOWN ? '\0' : (char)key);
				
				for (auto i = charEvents.begin(); i != charEvents.end(); i++)
					(*i)(key);
			}

			int initWithWindow(GLFWwindow* window) {
				glfwSetKeyCallback(window, press_call);
				glfwSetCharCallback(window, char_call);
				return 0;
			}
			...
			void addKeyEvent(void(*function)(int key, int scancode, int action, int mods)) {
				keyEvents.push_back(function);
			}

			void addCharEvent(void(*function)(unsigned int key)) {
				charEvents.push_back(function);
			}
