...
namespace neutron {
	namespace processor {
		...
		// Crop a texture
		void cropTexture(unsigned int *texture, int x, int y, int width, int height) {
			if (width == 0 || height == 0) { // Check size to make sure it's a possible image
				std::cout << "TEXTURE WIDTH OR HEIGHT IS 0 IN NEUTRON->PROCESSOR->CROPTEXTURE" << std::endl;
				return;
			}
			...
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0);	// Set texture to empty as 2d, with RGB and A channels, using unsigned unsigned char

			glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, *texture, 0);	// Set a Color attachment to the frame buffer
			glDrawBuffer(GL_COLOR_ATTACHMENT0);										// Set a Color attachment to the draw buffer
			glBindFramebuffer(GL_FRAMEBUFFER, FramebufferName);						// Bind the framebuffer once more

			unsigned int shader = LoadShader("res/shaders/crop.shader");
			glUseProgram(shader);						// Use the "colors to img" shader,
			glViewport(0, 0, width, height);			// Assign viewport to match the framebuffer size
			glClearColor(0.0, 0.0, 0.0, 0.0);			// Set background color to clear
			glClear(GL_COLOR_BUFFER_BIT);				// Use background color as background

			glm::vec2 size = sizeofTexture(*texture, GL_TEXTURE_2D);
			uniF4(shader, "crop", glm::vec4(x / size.x, y / size.y, width / size.x, height / size.y));
			glActiveTexture(GL_TEXTURE0);
			glBindTexture(GL_TEXTURE_2D, *texture);
			glBindVertexArray(dVAO);				// Bind VAO
		...
		// Crop a texture
		unsigned int getCroppedTexture(unsigned int texture, int x, int y, int width, int height) {
			if (width == 0 || height == 0) { // Check size to make sure it's a possible image
				std::cout << "TEXTURE WIDTH OR HEIGHT IS 0 IN NEUTRON->PROCESSOR->CROPTEXTURE" << std::endl;
				return 0;
			}
			...
			glBindTexture(GL_TEXTURE_2D, texture);
			int inMa, inMn, clS, clT;
			glGetTexParameteriv(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, &inMa);
			glGetTexParameteriv(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, &inMn);
			glGetTexLevelParameteriv(GL_TEXTURE_2D, 0, GL_TEXTURE_WRAP_S, &clS);
			glGetTexLevelParameteriv(GL_TEXTURE_2D, 0, GL_TEXTURE_WRAP_T, &clT);
			glBindTexture(GL_TEXTURE_2D, drawTex);
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, inMa);		// Use set interpolation close up
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, inMn);		// ..and far away
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, clS);	// Don't overlap horizontally
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, clT);	// ..or vertically
