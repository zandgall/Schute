...
namespace neutron {
	...
	int setUniforms(unsigned int texture, glm::vec4 color, float layer, unsigned int vertices) {
		...
		uniB(s, "ui", modes::gui);
		frame::width = std::max(frame::width, 1);
		frame::height = std::max(frame::height, 1);
		glm::mat4 ortho;
		if (modes::gui) {
			int viewSize[4];
	...
	void drawText_FLAT(const std::u32string &text, const char* font, unsigned int texture, glm::vec4 color, float layer, unsigned int vertices, float xspacing, float yspacing) {
		float x = 0, y = 0;
		...
		for (c = text.begin(); c != text.end(); c++) {
			...
			position = glm::vec4(2 * (x + ch.bearing.x + size.x / 2) / (dim.x - 0*10) - 1,
				2 * (y - ch.bearing.y + ch.size.y / 2 + (dim.y) / 2) / dim.y, 0, 0);
	...
	std::u32string removeBlankCharacters(std::u32string text) {
	...
	void drawText(const std::string& text, const char* font, unsigned int texture, glm::vec4 color, float layer, unsigned int vertices, float xspacing, float yspacing) {
		drawText(std::wstring_convert<std::codecvt_utf8<char32_t>, char32_t>().from_bytes(text), font, texture, color, layer, vertices, xspacing, yspacing);
	}
	void drawText(const std::u32string &text, const char* font, unsigned int texture, glm::vec4 color, float layer, unsigned int vertices, float xspacing, float yspacing) {
		using namespace passes;
		if (vertices == 0)
			vertices = ((vao == VAO_FLAT) ? 6 : (vao == VAO_CUBE) ? 36 : (vao == VAO_SPHERE) ? 192 : 0);
