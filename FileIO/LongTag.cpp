...
void LongTag::load(unsigned char* bytes, int index) {
	cout << "Loading short";
	...
	data = *(long long*)(new unsigned char[8]{ bytes[index + 7], bytes[index + 6], bytes[index + 5], bytes[index + 4], bytes[index + 3], bytes[index + 2], bytes[index + 1], bytes[index + 0] });
}

int LongTag::next(unsigned char* bytes, int index) {
	return index + 11 + (int)(bytes[index + 1] * 16 * 16) + (int)bytes[index + 2];
}

vector<unsigned char> LongTag::getBytesValue() {
