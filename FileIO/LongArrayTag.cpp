...
void LongArrayTag::load(unsigned char* bytes, int index) {
	...
	for (int i = 0; i < size; i++) {
		data.push_back(*(long long*)(new unsigned char[8]{ bytes[index + i * 8 + 11], bytes[index + i * 8 + 10], bytes[index + i * 8 + 9], bytes[index + i * 8 + 8], bytes[index + i * 8 + 7], bytes[index + i * 8 + 6], bytes[index + i * 8 + 5], bytes[index + i * 8 + 4] }));
	}
}

int LongArrayTag::next(unsigned char* bytes, int index) {
	return index + 7 + (int)(bytes[index + 1] * 16 * 16) + (int)bytes[index + 2] + data.size() * 8;
}

vector<unsigned char> LongArrayTag::getBytesValue() {
