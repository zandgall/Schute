#pragma once
#include "Tag.h"
class DoubleArrayTag : virtual public Tag {
public:
	DoubleArrayTag();
	DoubleArrayTag(std::string name, std::vector<double> data) {
		this->name = name;
		this->data = data;
	};
	DoubleArrayTag(std::string name, double data[]) {
		this->name = name;
		this->data = std::vector<double>();
		for (int i = 0, size = sizeof(data) / sizeof(double); i < size; i++)
	...
	void operator()(std::vector<double> b) {
		data = b;
	}
	...
	Tag* clone() {
		return new DoubleArrayTag(name, data);
	}
private:
	std::string name;
